...
import { isImageFile, isHeicFile } from "@/lib/studio/image-utils";
...
export function useImageUpload(options?: UseImageUploadOptions) {
  ...
  const handleImageUpload = async (file: File, imageNumber: 1 | 2) => {
    if (!validateImageFormat(file)) {
      showToast.current?.("请选择合法的图片文件", "error");
      return;
    }
    ...
    if (isHeic) {
      try {
        ...
        showToast.current?.("图片转换失败，请尝试其他格式", "error");
        return;
      }
    }
    ...
    reader.onerror = () => {
      showToast.current?.("读取图片失败，请重试", "error");
    };
  ...
  const restoreImageFromDataUrl = async (dataUrl: string, imageNumber: 1 | 2) => {
    try {
      ...
      console.error(`恢复图片 ${imageNumber} 失败:`, error);
    }
  };
